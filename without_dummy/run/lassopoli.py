"""
Polynomial LASSO Rolling Window Forecasts
"""
import os
import sys
import numpy as np
...
def main():
    ...
    for lag in range(1, 13):
        print(f"  PoliLASSO lag={lag}")
        results[f'polilasso{lag}c'] = polilasso_rolling_window(Y, nprev, indice=1, lag=lag)
        results[f'polilasso{lag}p'] = polilasso_rolling_window(Y, nprev, indice=2, lag=lag)
        rmse_cpi[lag] = results[f'polilasso{lag}c']['errors']['rmse']
        rmse_pce[lag] = results[f'polilasso{lag}p']['errors']['rmse']
    
    # Combine results for CPI (indice=1)
    cpi_polilasso = np.column_stack([results[f'polilasso{lag}c']['pred'] for lag in range(1, 13)])
    
    # Combine results for PCE (indice=2)
    pce_polilasso = np.column_stack([results[f'polilasso{lag}p']['pred'] for lag in range(1, 13)])
    
    # Create output directory
    os.makedirs(FORECAST_DIR, exist_ok=True)
    
    # Save forecasts
    save_forecasts(cpi_polilasso, os.path.join(FORECAST_DIR, 'polilasso-cpi.csv'))
    save_forecasts(pce_polilasso, os.path.join(FORECAST_DIR, 'polilasso-pce.csv'))
