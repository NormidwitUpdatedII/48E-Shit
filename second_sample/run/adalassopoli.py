"""
Adaptive LASSO with polynomial features run script for second sample
Runs Adaptive LASSO on polynomial-expanded data with dummy variable
"""
import sys
import os
import numpy as np
...
from second_sample.functions.func_polilasso import lasso_rolling_window
...
def main():
    ...
    print("Running Adaptive LASSO Polynomial forecasts (second sample with dummy)...")
    ...
    save_forecasts(cpi, os.path.join(FORECAST_DIR, 'adalassopoli-cpi.csv'))
    save_forecasts(pce, os.path.join(FORECAST_DIR, 'adalassopoli-pce.csv'))
    
    print(f"Adaptive LASSO Polynomial forecasts saved to {FORECAST_DIR}")
