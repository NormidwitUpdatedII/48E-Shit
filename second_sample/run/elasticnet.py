"""
Elastic Net forecasting run script for second sample
Runs elastic net with dummy variable for all lags and both indices
"""
import sys
import os
import numpy as np
...
def main():
    ...
    print("Running Elastic Net forecasts (second sample with dummy)...")
    ...
    save_forecasts(cpi, os.path.join(FORECAST_DIR, 'elasticnet-cpi.csv'))
    save_forecasts(pce, os.path.join(FORECAST_DIR, 'elasticnet-pce.csv'))
    
    print(f"Elastic Net forecasts saved to {FORECAST_DIR}")
