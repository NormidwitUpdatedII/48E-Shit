"""
LASSO with polynomial features run script for second sample
Runs LASSO on polynomial-expanded data with dummy variable
"""
import sys
import os
import numpy as np
...
def main():
    ...
    print("Running LASSO Polynomial forecasts (second sample with dummy)...")
    ...
    save_forecasts(cpi, os.path.join(FORECAST_DIR, 'lassopoli-cpi.csv'))
    save_forecasts(pce, os.path.join(FORECAST_DIR, 'lassopoli-pce.csv'))
    
    print(f"LASSO Polynomial forecasts saved to {FORECAST_DIR}")
